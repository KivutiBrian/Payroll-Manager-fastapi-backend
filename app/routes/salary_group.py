...
async def salary_groups(db:Session = Depends(get_db), current_user: UserOut = Depends(get_current_active_user)):
...
async def find_group(sgid:int, db:Session = Depends(get_db), current_user: UserOut = Depends(get_current_active_user)):
...
async def create_salary_group(payload:SalaryGroupCreate, db:Session = Depends(get_db), current_user: UserOut = Depends(get_current_active_user)):
...
async def update_salary_group(sgid:int, db:Session = Depends(get_db), current_user: UserOut = Depends(get_current_active_user)):
...
async def delete_salary_group(sgid:int, db:Session = Depends(get_db), current_user: UserOut = Depends(get_current_active_user)):
