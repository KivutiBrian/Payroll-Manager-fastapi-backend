...
async def employee_by_id(employee_id:int,db:Session = Depends(get_db) ):
...
async def create_employee(payload:CreateEmployee,db:Session = Depends(get_db) ):
...
async def edit_employee(employee_id:int, db:Session=Depends(get_db)):
    pass
